...
const ScanResult: React.FC<ScanResultProps> = ({ onFinish, onSecondaryClick, type, userData, eventData }) => {
  // Hitung stempel untuk kuliner
  let stamps = eventData 
    ? userData?.visitedDestinations?.filter((id: string) => id.startsWith(`${eventData.id}_kuliner_`)).length || 0
    : 0;
  
  // Optimistic update: jika tipe kuliner, tambah 1 karena data di userData mungkin belum update (karena navigasi instan)
  if (type === 'kuliner') {
    stamps = Math.min(stamps + 1, 5);
  }
  ...
  const content = {
    mission: {
      ...
      xp: "+500 XP Traveler",
      primaryBtn: "Lihat Wahana",
      primaryIcon: "attractions",
      secondaryBtn: "Selesai"
    },
    loket: {
      title: "Berhasil di Misi Event",
    ...
    kuliner: {
      title: "Berhasil Scan Stempel Kuliner",
      subtitle: "Stempel berhasil ditambahkan ke koleksimu.",
      xp: "+50 XP Kuliner",
      primaryBtn: "Selesai",
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-gray-100 flex items-center justify-center p-0 md:p-8 font-display text-[#1a1a1a] overflow-hidden">
      ...
      <div className="relative w-full h-full md:h-auto md:max-h-[850px] md:w-[420px] md:rounded-3xl bg-white shadow-2xl border-0 md:border md:border-white/50 flex flex-col">
        ...
        {/* Main Content */}
        <div className="flex-1 flex flex-col items-center justify-center pt-8 px-8 z-10 relative overflow-y-auto no-scrollbar">
                ...
                <div className="size-40 rounded-full bg-primary/5 flex items-center justify-center animate-pulse-soft">
                    <div className="size-28 rounded-full bg-gradient-to-br from-primary/10 to-primary/20 flex items-center justify-center border border-primary/10 shadow-inner">
                        <span className="material-symbols-outlined text-primary text-6xl" style={{ fontVariationSettings: "'FILL' 1" }}>
                          {type === 'kuliner' ? 'restaurant' : 'confirmation_number'}
                        </span>
                    </div>
                </div>
            ...
            {type === 'kuliner' ? (
              <div className="w-full bg-white rounded-2xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 relative overflow-hidden">
                 <div className="flex justify-between items-center mb-4">
                    <span className="text-sm font-bold text-gray-900">Koleksi Stempel</span>
                    <span className="text-primary font-black">{stamps}/{maxStamps}</span>
                 </div>
                 <div className="flex justify-between gap-2">
                    {Array.from({ length: maxStamps }).map((_, i) => (
                      <div key={i} className={`flex-1 aspect-square rounded-lg flex items-center justify-center border-2 transition-all ${
                        i < stamps 
              ...
              <div className="w-full bg-white rounded-2xl p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-3">
                          <div className="size-10 rounded-xl bg-gray-50 flex items-center justify-center text-primary border border-gray-100 shadow-sm">
                              <span className="material-symbols-outlined">
                                {type === 'loket' ? 'event' : 'flag'}
                              </span>
                          </div>
                          <div>
                              <h3 className="text-sm font-bold text-gray-900">{type === 'loket' ? 'Event Check-in' : 'Misi Wisata'}</h3>
                  ...
                  <div className="flex justify-between items-end mb-2 relative z-10">
                      {/* Hardcoded 2/4 as requested for Mission */}
                      <span className="text-2xl font-bold text-gray-900 leading-none">
                        {type === 'mission' ? '2' : '1'}
                        <span className="text-sm text-gray-500 font-medium ml-0.5">/4</span>
                      </span>
                  ...
                  <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden relative z-10">
                      <div className="h-full bg-gradient-to-r from-primary to-purple-400 rounded-full relative overflow-hidden" style={{ width: type === 'mission' ? '50%' : '25%' }}>
